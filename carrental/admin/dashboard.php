...
.panel {
    border-radius: 50px; /* Makes the panel oval */
    overflow: hidden; /* Ensures the content stays within the rounded corners */
}

/* Simple color backgrounds for each panel */
.panel-body.bk-primary {
    background-color: #ff0000; /* Red */
    border-radius: 50px;
}

.panel-body.bk-success {
...
.panel-body.bk-warning {
    background-color: #ffff00; /* Yellow */
    border-radius: 50px;
}

.panel-body.bk-danger {
    background-color: #ff0000; /* Red */
    border-radius: 50px;
}
...
?>
    ...
    <div class="panel panel-default">
        <div class="panel-body bk-danger text-light">
            <div class="stat-panel text-center">
                <div class="stat-panel-number h1" id="total-payments">
                    <?php
                    // Fetch the total count of payments (on page load)
                    $stmt = $dbh->query("SELECT COUNT(*) as total_count FROM payments");
        ...
        <a href="manage-payment.php" class="block-anchor panel-footer">Full Detail <i class="fa fa-arrow-right"></i></a>
    </div>
</div>
...
<div class="col-md-3">
    <div class="panel panel-default">
        <div class="panel-body bk-primary text-light">
            <div class="stat-panel text-center">
                <div class="stat-panel-number h1" id="total-revenue">
                    <?php
                    // Fetch the total revenue from the payments table
                    $stmt = $dbh->query("SELECT SUM(amount) as total_revenue FROM payments WHERE status = 'completed'");
                    $result = $stmt->fetch(PDO::FETCH_ASSOC);
                    echo number_format($result['total_revenue'], 2); // Format to 2 decimal places
                    ?>
                </div>
                <div class="stat-panel-title text-uppercase">Revenue Generated</div>
            </div>
        </div>
        <a href="revenue.php" class="block-anchor panel-footer">View Details <i class="fa fa-arrow-right"></i></a>
    </div>
</div>
